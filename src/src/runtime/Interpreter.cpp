...
Interpreter::Interpreter(const std::string& root) : projectRoot(root) {
    // Create global environment with shared_ptr for automatic lifetime management
...
void Interpreter::cleanup() {
    ...
    if (workerPools.empty() && goroutines.empty() && currentEnv == globalEnv) {
        return;
    }
    ...
    // Reset to global environment - local scopes will be automatically freed
    // when no closures hold references to them (shared_ptr reference counting)
    currentEnv = globalEnv;
...
RuntimeValue Interpreter::executeCallback(const RuntimeValue& closureValue, const std::vector<RuntimeValue>& args) {
    ...
    // Save current environment (shared_ptr copy keeps it alive)
