...
std::shared_ptr<Promise<void>> AsyncRuntime::delay(int milliseconds) {
    ...
    std::thread([promise, milliseconds]() {
        try {
            ...
            std::cerr << "[AsyncRuntime] Error in delay: " << e.what() << std::endl;
            try { promise->reject(e.what()); } catch (...) {}
        } catch (...) {
            std::cerr << "[AsyncRuntime] Unknown error in delay" << std::endl;
            try { promise->reject("Unknown error"); } catch (...) {}
        }
    }).detach();
...
std::shared_ptr<Promise<T>> AsyncRuntime::spawn(std::function<T()> task) {
    ...
    std::thread([promise, task]() {
        try {
            ...
            try { promise->reject("Unknown error in spawned task"); } catch (...) {}
        }
    }).detach();
...
void GoroutineRuntime::go(std::function<void()> func) {
    ...
    std::thread([this, func]() {
        try {
            ...
            std::cerr << "[GoroutineRuntime] Error in goroutine: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "[GoroutineRuntime] Unknown error in goroutine" << std::endl;
        }
        activeCount_--;
    }).detach();
}
