...
public class StratosLexer extends MergingLexerAdapterBase {
    ...
    private static FlexAdapter createLexer() {
        return new FlexAdapter(new _StratosLexer() {
            @Override
    ...
    private static final MergeFunction MERGE_FUNCTION = (firstTokenType, originalLexer) -> {
        if (firstTokenType == REGULAR_STRING_PART || firstTokenType == WHITE_SPACE) {
            ...
            return firstTokenType == MULTI_LINE_DOC_COMMENT_START ? MULTI_LINE_COMMENT
                    : MULTI_LINE_COMMENT;
        }
