...
//  }
  ...
  private static boolean validDartSdkPath(String path) {
    return path != null &&
           (path.endsWith(StratosSdk.DART_SDK_SUFFIX) ||
            path.endsWith(StratosSdk.LINUX_DART_SUFFIX) ||
            path.endsWith(StratosSdk.MAC_DART_SUFFIX));
  }
...
//  }
  ...
  public static boolean hasFlutterModule(@NotNull Project project) {
    System.out.println("======>>> flutter module fixx me "+project.getName());
    return true;
...
//    return CollectionUtils.anyMatch(getModules(project), FlutterModuleUtils::isFlutterModule);
  ...
  public static boolean isInFlutterModule(@NotNull PsiElement element) {
    System.out.println("======>>> flutter module fixx me "+element.getText());
    return true;
