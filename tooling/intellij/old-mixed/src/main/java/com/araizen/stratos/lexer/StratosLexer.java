...
public class StratosLexer extends LexerBase {
    ...
    private final _StratosLexer myFlexLexer;
    ...
    public StratosLexer(@NotNull LanguageLevel level) {
        myFlexLexer = new _StratosLexer(level);
    }
        ...
        switch (c) {
            ...
            case '/':
                if (myBufferIndex + 1 >= myBufferEndOffset) {
                    myTokenType = StratosTokenType.DIV;
                    myTokenEndOffset = myBufferEndOffset;
                } else {
                    char nextChar = charAt(myBufferIndex + 1);
                    if (nextChar == '/') {
                        myTokenType = StratosTokenType.END_OF_LINE_COMMENT;
                        ...
                        if (myBufferIndex + 2 >= myBufferEndOffset ||
                            ...
                            myTokenType = StratosTokenType.C_STYLE_COMMENT;
                            myTokenEndOffset = getClosingComment(myBufferIndex + 2);
                        } else {
                            myTokenType = StratosDocElementType.DOC_COMMENT;
            ...
            case '#':
                ...
                break;
            case '\'':
                myTokenType = StratosTokenType.CHARACTER_LITERAL;
            ...
            case '"':
                if (myBufferIndex + 2 < myBufferEndOffset && charAt(myBufferIndex + 2) == '"' && charAt(myBufferIndex + 1) == '"') {
                    myTokenType = StratosTokenType.TEXT_BLOCK_LITERAL;
                    myTokenEndOffset = getTextBlockEnd(myBufferIndex + 2);
                } else {
                    myTokenType = StratosTokenType.STRING_LITERAL;
                    myTokenEndOffset = getClosingQuote(myBufferIndex + 1, c);
                }
                break;
