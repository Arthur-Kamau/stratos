...
public final class StratosParserUtil {
    public static final TokenSet WS_COMMENTS = TokenSet.orSet(StratosElementType.JAVA_COMMENT_BIT_SET, TokenSet.WHITE_SPACE);
    private static final Key<LanguageLevel> LANG_LEVEL_KEY = Key.create("StratosParserUtil.LanguageLevel");
    private static final Key<Boolean> DEEP_PARSE_BLOCKS_IN_STATEMENTS = Key.create("StratosParserUtil.ParserExtender");

    @NotNull
    public static TokenList obtainTokens(@NotNull PsiFile file) {
        return CachedValuesManager.getCachedValue(file, () ->
                CachedValueProvider.Result.create(
                        TokenSequence.performLexing(file.getViewProvider().getContents(), StratosParserDefinition.createLexer(PsiUtil.getLanguageLevel(file))),
                        file));
    }
    ...
    private static class PrecedingWhitespacesAndCommentsBinder implements WhitespacesAndCommentsBinder {
        ...
        public int getEdgePosition(final List<IElementType> tokens, final boolean atStreamEdge, final TokenTextGetter
            ...
            for (int idx = tokens.size() - 1; idx >= 0; idx--) {
                if (tokens.get(idx) == StratosDocElementType.DOC_COMMENT) return idx;
            }
            ...
            for (int idx = tokens.size() - 1; idx >= 0; idx--) {
                ...
                else if (StratosElementType.JAVA_PLAIN_COMMENT_BIT_SET.contains(tokenType)) {
                    if (atStreamEdge ||
    ...
    private static final TokenSet PRECEDING_COMMENT_SET = TokenSet.orSet(
            TokenSet.create(ElementType.MODULE), StratosElementType.FULL_MEMBER_BIT_SET);

    private static final TokenSet TRAILING_COMMENT_SET = TokenSet.orSet(
            TokenSet.create(JavaElementType.PACKAGE_STATEMENT),
            StratosElementType.IMPORT_STATEMENT_BASE_BIT_SET,
            StratosElementType.FULL_MEMBER_BIT_SET, StratosElementType.JAVA_STATEMENT_BIT_SET);
    ...
    public static PsiBuilder createBuilder(final ASTNode chameleon) {
        ...
        Lexer lexer = psi instanceof PsiFile && indexedText != null ? obtainTokens((PsiFile)psi).asLexer()
                : StratosParserDefinition.createLexer(level);
        Language language = psi.getLanguage();
        if (!language.isKindOf(StratosLanguage.INSTANCE)) language = StratosLanguage.INSTANCE;
    ...
    public static PsiBuilder createBuilder(final LighterLazyParseableNode chameleon) {
        ...
        final Lexer lexer = StratosParserDefinition.createLexer(level);
    ...
    public static ASTNode parseFragment(final ASTNode chameleon, final ParserWrapper wrapper, final boolean eatAll, final LanguageLevel level) {
        ...
        final PsiBuilderFactory factory = PsiBuilderFactory.getInstance();
        final Lexer lexer = chameleon.getElementType() == StratosDocElementType.DOC_COMMENT
                ? StratosParserDefinition.createDocLexer(level) : StratosParserDefinition.createLexer(level);
    ...
    public static void semicolon(final PsiBuilder builder) {
        expectOrError(builder, StratosTokenType.SEMICOLON, "expected.semicolon");
    }
    ...
    public static PsiBuilder braceMatchingBuilder(final PsiBuilder builder) {
        ...
        while (!builder.eof()) {
            ...
            if (tokenType == StratosTokenType.LBRACE) braceCount++;
            else if (tokenType == StratosTokenType.RBRACE) braceCount--;
