...
%struct.FileInfo = type { i8*, i32, i1, i32 }
...
define %struct.FileInfo* @FileInfo() {
  %t12 = call i8* @malloc(i64 32)
  %t13 = bitcast i8* %t12 to %struct.FileInfo*
  %t14 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %t13, i32 0, i32 0
  store i8* null, i8** %t14
  %t15 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %t13, i32 0, i32 1
  store i32 0, i32* %t15
  %t16 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %t13, i32 0, i32 2
  store i1 0, i1* %t16
  %t17 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %t13, i32 0, i32 3
  store i32 0, i32* %t17
  ret %struct.FileInfo* %t13
}


; Method FileInfo.getName
define i8* @FileInfo_getName(%struct.FileInfo* %this_ptr) {
  %t0 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %this_ptr, i32 0, i32 0
  %t1 = load i8*, i8** %t0
  ret i8* %t1
}


; Method FileInfo.getSize
define i32 @FileInfo_getSize(%struct.FileInfo* %this_ptr) {
  %t0 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %this_ptr, i32 0, i32 1
  %t1 = load i32, i32* %t0
  ret i32 %t1
  ret i32 0
}
...
; Method FileInfo.getIsDir
define i1 @FileInfo_getIsDir(%struct.FileInfo* %this_ptr) {
  %t0 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %this_ptr, i32 0, i32 2
  %t1 = load i1, i1* %t0
  ret i1 %t1
}


; Method FileInfo.getModTime
define i32 @FileInfo_getModTime(%struct.FileInfo* %this_ptr) {
  %t0 = getelementptr inbounds %struct.FileInfo, %struct.FileInfo* %this_ptr, i32 0, i32 3
  %t1 = load i32, i32* %t0
  ret i32 %t1
  ret i32 0
}
...
define i32 @listFiles(i8* %arg0, i32 %arg1) {
  ...
  %t5 = load i32, i32* %filtered_3
  ret i32 %t5
  ret i32 0
}


define i32 @listFilesOnly(i8* %arg0) {
  %path.addr = alloca i8*
  store i8* %arg0, i8** %path.addr
  ret i32 0
}
...
define i32 @listDirectoriesOnly(i8* %arg0) {
  %path.addr = alloca i8*
  store i8* %arg0, i8** %path.addr
  ret i32 0
}
...
define i1 @ensureDir(i8* %arg0) {
  %path.addr = alloca i8*
  store i8* %arg0, i8** %path.addr
  %t0 = load i8*, i8** %path.addr
  %t1 = call i32 @exists(i8* %t0)
  %t2 = icmp ne i32 %t1, 0
  br i1 %t2, label %L6, label %L7

L6:
  %t3 = load i8*, i8** %path.addr
  %t4 = call i32 @isDirectory(i8* %t3)
  %t5 = icmp ne i32 %t4, 0
  br i1 %t5, label %L9, label %L10
...
L8:
  ...
  %t7 = call i32 @mkdirAll(i8* %t6)
  ret i32 %t7
}


define i1 @walk(i8* %arg0, i32 %arg1) {
  %root.addr = alloca i8*
  store i8* %arg0, i8** %root.addr
  %visitor.addr = alloca i32
  store i32 %arg1, i32* %visitor.addr
  %files_0 = alloca i32
  %t1 = load i8*, i8** %root.addr
  %t2 = call i32 @readDir(i8* %t1)
  store i32 %t2, i32* %files_0
  ; For loop not yet implemented in IR generation
  ret i1 1
...
@.str0 = private unnamed_addr constant [30 x i8] c"Called unwrap on Error result\00"
