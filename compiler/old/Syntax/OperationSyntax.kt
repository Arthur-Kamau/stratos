...
import com.stratos.old.Statement.OperationStatement
...
class OperationSyntax : Syntax() {
    ...
    fun execute(token: Token): com.stratos.old.Statement.Statement? {
...
//            null
    ...
    private fun simpleExpression(): OperationStatement? {
        val node = advance()
        return if (  node.type == NodeType.SignedFloatNode || node.type == NodeType.UnSignedFloatNode || node.type == NodeType.SignedNumericNode || node.type == NodeType.UnSignedNumericNode ) {
    ...
    fun parseExpression(): List<OperationStatement>? {
        ...
        val node = advance()
        if (  node.type == NodeType.SignedFloatNode || node.type == NodeType.UnSignedFloatNode || node.type == NodeType.SignedNumericNode || node.type == NodeType.UnSignedNumericNode ) {
            if (peek().type == NodeType.AddNode || peek().type == NodeType.SubtractNode || peek().type == NodeType.ModulasNode || peek().type == NodeType.MultiplyNode) {
                val operation = advance()
                val node2 = advance()
