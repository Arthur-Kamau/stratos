package old.stratos.analysis.antlr;
 ...
 */
public abstract class StratosParserBase extends Parser
{
    protected StratosParserBase(TokenStream input) {
        super(input);
    }
    ...
    protected boolean lineTerminatorAhead() {
        ...
        if (ahead.getType() == StratosLexer.TERMINATOR) {
            // There is definitely a line terminator ahead.
            return true;
        }

        if (ahead.getType() == StratosLexer.WS) {
        ...
        return (type == StratosLexer.COMMENT && (text.contains("\r") || text.contains("\n"))) ||
                (type == StratosLexer.TERMINATOR);
    }
    ...
    protected boolean noTerminatorAfterParams(int tokenOffset) {
        ...
        if (stream.LT(tokenOffset).getType() == StratosLexer.L_PAREN) {
            // Scan past parameters
            while (leftParams != rightParams) {
                tokenOffset++;
                valueType = stream.LT(tokenOffset).getType();

                if (valueType == StratosLexer.L_PAREN){
                    leftParams++;
                }
                else if (valueType == StratosLexer.R_PAREN) {
                    rightParams++;
                }
            }
